function Build(settings, patterns) {
  ...
  str += '\n\n'
  str += ':root{\n'
  str += '\t--offset{{=}}' + resources.settings.offset + '{{;}}\n'
  for (let name in resources.settings.breakPoints) {
    let br = resources.settings.breakPoints[name]

    if (br.offset !== undefined) {
      str += '\t--offset-' + name + '{{=}}' + br.offset + '{{;}}\n'
      // last_offset = "{{var}}offset_" + name;
      // last_offset = "{{var}}offset_" + name;
    } else {
      // str += "{{var}}offset_" + name + '{{=}}' + last_offset + "{{;}}\n";
      str += '\t--offset-' + name + '{{=}}' + resources.settings.offset + '{{;}}\n'
    }
  }

  let offsetGet = resources.settings.offset.split(/(\d+)/)
  str += '\t--offset-one-side{{=}}' + offsetGet[1] / 2 + offsetGet[2] + '{{;}}\n'
  for (let name in resources.settings.breakPoints) {
    str += `\t--offset-${name}-one-side{{=}}calc(var(--offset-${name}) / 2){{;}}\n`
  }

  str += '\t--fields{{=}}' + resources.settings.baseFields + '{{;}}\n'
  for (let name in resources.settings.breakPoints) {
    let br = resources.settings.breakPoints[name]

    if (br.fields !== undefined) {
      str += '\t--fields-' + name + '{{=}}' + br.fields + '{{;}}\n'
    }
  }
  str += '}\n'
  str += '\n\n'
  ...
  let uSizeCommands = resources.styles.objToStyles(
    {
      ...
      width:
        '{{i}}calc(100% / {{string-var}}columns{{/string-var}} * {{string-var}}n{{/string-var}} - var(--offset)){{/i}}',
    },
    1
  )
