            ...
            if(this.resources.helpers.isPercentage(this.resources.settings.offset)){
                for(let j = 0; j < this.map[i].props.length; j++){
                    ...
                    styles += this.resources.styles.objToCallMedia(this.postfix, {
                        ...
                        [this.map[i].props[j]]: `{{var}}atom * {{var}}n + var(--offset${mediaPostfix.replace('_', '-')}-one-side)`
                    });
                }
            }
                ...
                styles += `{{var}}val{{=}}{{i}}calc(100% / {{string-var}}columns{{/string-var}} * {{string-var}}n{{/string-var}} + var(--offset${mediaPostfix.replace('_', '-')}-one-side)){{/i}}{{;}}\n`;
                ...
                for(let j = 0; j < this.map[i].props.length; j++){
                    ...
                    styles += this.resources.styles.objToCallMedia(this.postfix, {
                        ...
                        [this.map[i].props[j]]: `{{i}}calc({{string-var}}val{{/string-var}} + var(--offset${mediaPostfix.replace('_', '-')}-one-side)){{/i}}`
                    });
                }
            }
